...
export class YearData implements IYearData {
  ...
  generateNextYearInput(config: TaxConfig): IYearInputData {
    ...
    newInput.cipNewAssessedGrowth =
      newInput.cipTotalAssessed * config.cipNewGrowthRate;
    newInput.cipTotalAssessed =
      newInput.cipTotalAssessed * (1 + config.cipGrowthRate);

    newInput.roTotalAssessed =
      newInput.roTotalAssessed * (1 + config.roGrowthRate);
    newInput.roNewAssessedGrowth =
      newInput.roTotalAssessed * config.roNewGrowthRate;
  ...
  get cipTaxRate(): number {
    return (
      1000 * (this.cipShiftedPercent * this.levyLimit) / this.cipTotalAssessed
    );
  }
  get cipTaxRateWithBond(): number {
    return (
      1000 *
      (this.cipShiftedPercent * this.levyLimitWithBond) /
      this.cipTotalAssessed
    );
  }
  ...
  get roTaxRateWithBond(): number {
    return (
      1000 *
      (this.roShiftedPercent * this.levyLimitWithBond) /
      this.roTotalAssessed
    );
  }
  get roTaxRate(): number {
    return (
      1000 * (this.roShiftedPercent * this.levyLimit) / this.roTotalAssessed
    );
  }
  get levyLimit(): number {
    let newLimit = this._currentYear.priorYearLevyLimit * 1.025;
    newLimit +=
      this.cipNewAssessedGrowth *
      (this._currentYear.priorYearCipTaxRateWithBond / 1000);
    newLimit +=
      this.roNewAssessedGrowth *
      (this._currentYear.priorYearRoTaxRateWithBond / 1000);
    return newLimit;
  }
  get levyLimitGrowthPercent(): number {
    return (
      (this.levyLimit - this._currentYear.priorYearLevyLimit) /
      this._currentYear.priorYearLevyLimit
    );
  }
  ...
  get homeYearOverYearIncrease(): number {
    if (this._previousYear.homeTarget === 0) {
      return 0;
    }
    return (
      this.homeTargetTaxBill -
      this._previousYear.homeTarget *
        this._currentYear.priorYearRoTaxRateWithBond /
        1000
    );
  }
  ...
  get debtExclusionGrowthDifference(): number {
    ...
    newGrowth +=
      this.cipNewAssessedGrowth *
      (this._currentYear.priorYearCipTaxRate / 1000);
    newGrowth +=
      this.roNewAssessedGrowth * (this._currentYear.priorYearRoTaxRate / 1000);
  ...
  get bondRequirement(): number {
    ...
    if (this._config.enableBondPaymentRecycling) {
      paymentFromDebtServices += this.sumDebtExclusionGrowthDifference;
    }
    if (paymentFromDebtServices > this._currentYear.bondRequirement) {
      return 0;
    }
